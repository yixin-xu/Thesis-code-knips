...
source(paste0(code_directory, "/convert_transition_matrices_to_df.R"))
source(paste0(code_directory, "/generate_state_qalys.R"))
source(paste0(code_directory, "/generate_state_costs.R"))
source(paste0(code_directory, "/generate_net_benefit_cpp_full.R"))
...
if (initial_age == "0-") {finial_age <- 55}else{finial_age <- substring(age_range, 4, 6)}
...
results_matrix = cbind(results_matrix, round(model_outputs$ICER,2))
colnames(results_matrix)  = c( "QALYs", "Costs", "Net_benefit", "Inc.Net_benefit", "ICER")

for (treatment in treatment_names){
  if(results_matrix[treatment,5]<0) {
    if (mean(model_outputs$total_qalys[treatment,])-mean(model_outputs$total_qalys[1,])>0){results_matrix[treatment,5] = "Dominant"}
    if (mean(model_outputs$total_qalys[treatment,])-mean(model_outputs$total_qalys[1,])<0){results_matrix[treatment,5] = "Dominated"} 
  }
