...
generate_model_inputs <- function(n_samples, age_range, sample_gender, use_dummy_data = FALSE) {
  ...
  # Transition 1
  # From Post TKR to TEMP revision
  ...
  mr <- as.matrix(lifetables[85:100, paste0(sample_gender, "s")])
  mr_times <- c(0:15)}
  if(!is.infinite(age_range[2])) {
  ...
  input_parameters <- cbind(input_parameters, log_rate_middle_second, log_rate_late_second)
  input_parameters <- cbind(input_parameters, probability_higher_revision)
  colnames(input_parameters) <- c("log_rate_early_second", "log_rate_middle_second", "log_rate_late_second", "probability_higher_revision")
  input_parameters <- cbind(input_parameters, probability_higher_revision)
  for(implant_name in implant_names) {
