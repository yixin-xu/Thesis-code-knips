...
if(is.infinite(age_range[2])) {
  starting_age <- 90}else{starting_age <- ceiling(mean(age_range))}
if(age_range[2]==55){starting_age <- 53}
if(age_range[2]==55){
  initial_age <- 53}else{initial_age <- ceiling(mean(age_range))}
final_age <- 100
...
implant_names <- implant_costs_raw$implant_name[1:12]

source(paste0(code_directory, "/knips_functions.R"))
source(paste0(code_directory, "/cost-utility.R"))
source(paste0(code_directory, "/input.R"))
source(paste0(code_directory, "/outputs.R"))
...
states <- data.table( # Non-other cause death health states
  state_id = 1:7,
  state_name = state_names[1:7]#, "Surgery Death")
) 
